...
class NoteOff():
    ...
    def __init__(self, note_number, velocity):
        ...
        if not isinstance(note_number, int) or not isinstance(velocity, int):
            ...
            raise ValueError(
                'Specified note number is out of range. Must be set from 0 '
                'to 127 ({} given).'.format(note_number))
        elif not 0 <= velocity <= 127:
            raise ValueError(
                'Specified velocity for note number {} is out of range. '
                'Must be set from 0 to 127 ({} given).'.format(
...
class NoteOn():
    ...
    - velocity
     ...
     NB: a message type 'Note On' with velocity set to 0 will be read as
     'Note Off' on any MIDI device.
    """
...
class PolyphonicAftertouch():
    ...
    NB: this type of message is not so common when reading, because it is
    ...
    def __init__(self, note_number, pressure):
        ...
        if not isinstance(note_number, int) or not isinstance(pressure, int):
            ...
            raise ValueError(
                'Specified pressure value for note number {} is out of range. '
                'Must be set from 0 to 127 ({} given).'.format(
...
class ChannelAftertouch():
    ...
    def __init__(self, pressure):
        ...
        if not isinstance(pressure, int):
            raise TypeError(
                'Program number must be type int ({} given)'.format(
                    type(pressure)))
        elif not 1 <= pressure <= 128:
            raise ValueError(
                'Specified pressure value is out of range. Must be set from'
                ' 0 to 127 ({} given).'.format(pressure))
...
class ControlChange():
    ...
    def __init__(self, number, value):
        ...
        if not isinstance(number, int) or not isinstance(value, int):
            ...
            raise ValueError(
                'Specified control number is out of range. Must be set from 0'
                ' to 127 ({} given).'.format(number))
        elif not 0 <= value <= 127:
            raise ValueError(
                'Specified value for control number {} is out of range.'
                ' Must be set from 0 to 127 ({} given).'.format(number, value))
...
class ProgramChange():
    ...
    def __init__(self, number):
        ...
        if not isinstance(number, int):
            ...
            raise ValueError(
                'Specified program number is out of range. Must be set from'
                ' 1 to 128 ({} given).'.format(number))
...
class PitchBend():
    ...
    def __init__(self, lsbyte, msbyte):
        ...
        if not isinstance(lsbyte, int) or not isinstance(msbyte, int):
            ...
            raise ValueError(
                'Specified 1st byte value is out of range. Must be set from 0 '
                'to 127 ({} given).'.format(lsbyte))
        elif not 0 <= msbyte <= 127:
            raise ValueError(
                'Specified 2nd byte value is out of range. Must be set from 0 '
                'to 127 ({} given).'.format(msbyte))
        ...
        if args:
            for arg in args:
                if not isinstance(arg, int):
                    raise TypeError(
                        'All data transmitted through SysEx must be '
                        'integers ({} given)'.format(type(arg)))
                else:
